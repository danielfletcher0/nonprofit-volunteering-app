...
// Simulating an in-memory data store for profiles
let profiles = [];
...
const validateProfile = (profile) => {
    ...
    // Address 2 Validation (Optional, but must be less than 100 characters if provided)
    ...
    // Zip Code Validation (Between 5 and 9 characters)
    if (!profile.zip || profile.zip.length < 5 || profile.zip.length > 9) {
        errors.push('Zip Code must be between 5 and 9 characters.');
    }

    // Skills Validation (Must be an array and have at least one skill)
    if (!Array.isArray(profile.skills) || profile.skills.length === 0) {
        errors.push('At least one skill is required.');
    }

    // Availability Validation (Must be an array and have at least one date)
...
// Create a new profile (POST /profile/create)
router.post('/create', (req, res) => {
    ...
    // Add the new profile to the in-memory profiles array
    newProfile.id = profiles.length;  // Assign a simple unique ID (the index in the array)
...
// Get a specific profile by ID (GET /profile/:id)
router.get('/:id', (req, res) => {
    const profile = profiles.find(p => p.id == req.params.id);  // Find profile by ID
...
router.delete('/:id', (req, res) => {
    const profileIndex = profiles.findIndex(p => p.id == req.params.id);  // Find profile by ID
